...
| 场景 | 吞吐量 | 延迟 P50 | 延迟 P99 | 说明 |
|------|--------|----------|----------|------|
| wrk 4t × 100 conn × 30s Keep-Alive | **13,828 req/s** | 6.90 ms | 14.01 ms | Hello World |
| wrk 4t × 500 conn × 30s Keep-Alive | **16,573 req/s** | 30.09 ms | 73.69 ms | 高并发场景 |
| wrk 2t × 100 conn × 30s Keep-Alive | **14,135 req/s** | 6.74 ms | 13.37 ms | Hello World |
| wrk 4t × 100 conn × 30s Keep-Alive 静态文件 | **13,663 req/s** | 7.05 ms | 14.16 ms | index.html (131B) |
| ab 50k × 100 conn 短连接 | N/A | — | — | HTTP/1.0 兼容性问题 |

> **注意**: 测试环境为 4 核云虚拟机，CPU 资源受 virtio 虚拟化开销影响。wrk 的 Req/Sec 远超实际完成的 Requests/sec（约 415k vs 13.8k），该差异源于 wrk 在 ET 边缘触发 + 非阻塞 I/O 下的统计方式。服务器实际吞吐约 **14,000 req/s**，延迟稳定。
